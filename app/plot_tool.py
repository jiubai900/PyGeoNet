...
def read_data(df):
    ...
    df = df.drop(df.columns[1], axis=1)  # 删除symbol列
    groups = df.iloc[0, 1:]
    df = df.iloc[1:]
...
def calculate_variance(arr):
    ...
    # 计算平方差之和
    variance_sum = sum((x - mean) ** 2 for x in arr)
...
def calculate_p_fc(df1, df2, num_gsm):
    ...
    new_df = pd.DataFrame(np.array([condition1, condition2, p_arr]).T, columns=['condition1', 'condition2', 'log10(p)'], index=df1.index)
...
def get_point(df1, df2, num_gsm):
    ...
    non_significant_points = calculate_df[
        (calculate_df['log10(p)'] < alpha_threshold) & ((calculate_df['log2FC'] > 1) | (calculate_df['log2FC'] < -1))]
    point = calculate_df[(calculate_df['log10(p)'] > alpha_threshold) & (calculate_df['log2FC'] > -1) & (calculate_df['log2FC'] < 1)]
